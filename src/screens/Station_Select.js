...
class Station_Select extends Component {
	...
	next = () => {
		if (this.state.stationSel) {
			this.props.history.push('/Form', { station: this.state.stationSel });
		} else {
	...
	getLocGmap = async () => {
		try {
			...
			let res = await fetch(`http://localhost:8000/api/pioneer/address/${address},%20${state}`, {
				method: 'GET',
				headers: {
					'Accept': 'application/json, text/plain, */*',
					'Content-Type': 'application/json'
	...
	get = async (pos) => {
		try {
			...
			let res = await (await fetch(`http://192.168.43.94:8000/api/pioneer/stations/${pos.lat}-${pos.long}`,
				{
					headers: {
						'Accept': 'application/json, text/plain, */*',
						'Content-Type': 'application/json'
	...
	render() {
		let dropdown = <div className='dropdown' >
			<div className="searchbar">
				...
				<input onChange={(ev) => { this.setState({ address: ev.target.value }) }} style={{ flexGrow: 2 , padding:'7px'}} placeholder='Please enter your region ex.Ipodo,Festac, Alaba' />
				<div>
